...
export const getRecentlyViewItem = async({pidArray}) =>{
  ...
  const sql = `select pid, concat('http://54.180.8.46:9000/',JSON_UNQUOTE(JSON_EXTRACT(upload_img, '$[0]'))) as upload_img 
               from product 
...
export const getWishListInfo = async({id})=>{
  ...
  const sql =`
      select  P.pid
              ...
              , concat('http://54.180.8.46:9000/',JSON_UNQUOTE(JSON_EXTRACT(P.upload_img, '$[0]'))) as image_url
